    ...
    Box::new(
        move |f| {
            let paragraph = Paragraph::new(vec![Spans::from(vec![
                ...
                Span::styled(state.user_name.to_string(), Style::default().fg(Color::Yellow))
...
fn handle_char_press<B: 'static + Backend>(c: char) -> Box<dyn Fn(&mut State) -> Option<Window<B>>> {
    Box::new(
...
pub fn create_user_window<B: 'static + Backend>(_: &mut State) -> Option<Window<B>> {
    ...
    commands.insert(KeyCode::Backspace, WindowCommand {
        activator_key: KeyCode::Backspace,
        action: Box::new(handle_backspace_press)
    });
    commands.insert(KeyCode::Enter, WindowCommand {
